...
from rclpy.node import Node as NodeRos
...
class Map():
    ...
    def __open_map(self,map_name):
        ...
        #package_path = get_package_share_directory('task_4')
        package_path = '/home/me597/me597-tgobel/ros2_ws/Lab3_ws/task_4'
        print(package_path)
        map_yaml_path = os.path.join(package_path, 'maps', map_name + '.yaml')

        with open(map_yaml_path, 'r') as file1:
            f = yaml.safe_load(file1)

        #map_dir = os.path.abspath('/home/me597-tgobel/ros2_ws/Lab3_ws/task_4/maps/')
        #map_file = os.path.join(map_dir, map_name + '.yaml')
        #f = open(map_file, 'r')
        map_df = pd.json_normalize(f)
        # Open the map image
        map_name = map_df.image[0]
        
        map_pgm_path = os.path.join(package_path, 'maps', map_name)
        im = Image.open(map_pgm_path)
...
class AStar():
    def __init__(self,in_tree):
        ...
        for name,node in in_tree.g.items():
            ...
            end = tuple(map(int, self.in_tree.end))  # No need for .split(',')
...
class Navigation(NodeRos):
    ...
    def a_star_path_planner(self, start_pose, end_pose):
        ...
        mp.map_graph.root = start_pose.pose.position.x, start_pose.pose.position.y
        mp.map_graph.end = end_pose.pose.position.x, end_pose.pose.position.y
    ...
    def path_follower(self, vehicle_pose, current_goal_pose):
        ...
        # Calculating euler from quaternion using only numpy
        siny_cosp = 2 * (vehicle_pose.pose.position.w * vehicle_pose.pose.position.z + vehicle_x * vehicle_y)
        cosy_cosp = 1 - 2 * (vehicle_y **2 + vehicle_pose.pose.position.z**2)
